...
test.describe(`${siteName} - Add instance`, () => {
  ...
  test.describe('instance creation', () => {
    ...
    test('rejects a duplicate slug within the same site', async ({ authenticatedPage }) => {
      ...
      // The title's auto-generated slug kicks off its own availability check; if that
      // unrelated "available" response resolves after the real duplicate-slug check
      // below, it can overwrite the "already taken" error and clear it. Retry the
      // slug entry until the taken error actually sticks.
      await expect(async () => {
        await form.fillSlug(duplicateSlugInputs.slug);
        await form.blurSlugInput();
        await expect(authenticatedPage.getByText(messages.instances.slugAlreadyTaken)).toBeVisible({ timeout: 2_000 });
      }).toPass({ timeout: 15_000 });

      await expect(form.createButton).toBeDisabled();
      await form.closeForm();
      await expect(authenticatedPage.getByText(duplicateSlugInputs.secondTitle, { exact: true })).not.toBeVisible();
    });
